...
{
    /// <summary>
    /// 주방 업그레이드 요청, 재료 검사, 재료 소모, 레벨 증가를 담당합니다.
    /// 실제 Player Inventory가 완성되기 전까지는 WorkbenchTestInventory를 IInventory 대체 구현으로 사용합니다.
    /// </summary>
    [DisallowMultipleComponent]
    [RequireComponent(typeof(FacilityBase))]
    public sealed class KitchenUpgradeController : NetworkBehaviour
    {
        [Header("주방 시설")]
        [SerializeField]
        [Tooltip("업그레이드할 주방 시설입니다. 비워두면 같은 오브젝트에서 FacilityBase를 자동으로 찾습니다.")]
        private FacilityBase kitchenFacility;

        [SerializeField]
        [Tooltip("주방 업그레이드 재료가 들어 있는 Kitchen_Facility SO입니다.")]
        private FacilityDataSO kitchenFacilityData;

        [SerializeField]
        [Tooltip("주방 레벨 변경 후 최대 스태미너 보너스를 다시 계산할 컨트롤러입니다.")]
        ...
        private bool TryGetNextLevelData(out FacilityLevel nextLevelData)
        {
            ...
            if (kitchenFacility == null || kitchenFacilityData == null)
                return false;
        ...
        {
            return allowOfflineTestUpgrade && kitchenFacility != null && !kitchenFacility.IsSpawned;
        }
        ...
        private void PrintRequiredMaterials(FacilityLevel levelData, IInventory inventory)
        {
            ...
            for (int i = 0; i < levelData.upgradeMaterials.Count; i++)
            {
                ...
                Debug.Log(
                    $"[KitchenUpgradeController] 필요 재료: {material.item.displayName}({material.item.itemID}) " +
        ...
        private void DebugUpgrade()
        {
            ...
            {
                Debug.LogWarning("[KitchenUpgradeController] Play Mode에서만 테스트할 수 있습니다.", this);
                return;
            }
