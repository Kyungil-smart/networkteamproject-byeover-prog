...
{
    /// <summary>
    /// 주방 업그레이드 재료 검사, 재료 소모, 시설 레벨 증가를 담당합니다.
    /// Facilities.cs의 Kitchen 클래스는 수정하지 않고, FacilityBase와 Kitchen_Facility 데이터 기준으로 동작합니다.
    /// </summary>
    [DisallowMultipleComponent]
    [RequireComponent(typeof(FacilityBase))]
    public class KitchenUpgradeController : NetworkBehaviour
    {
        [Header("주방 시설")]
        [SerializeField]
        [Tooltip("업그레이드할 주방 시설입니다. 비워두면 같은 오브젝트의 FacilityBase를 자동으로 찾습니다.")]
        private FacilityBase kitchenFacility;

        [SerializeField]
        [Tooltip("주방 스테미너 보너스를 계산하는 컨트롤러입니다. 비워두면 같은 오브젝트에서 자동으로 찾습니다.")]
        private KitchenStaminaBonusController staminaBonusController;

        [SerializeField]
        [Tooltip("주방 시설에 연결된 것과 같은 Kitchen_Facility SO를 넣습니다.")]
        private FacilityDataSO kitchenFacilityData;
        ...
        [SerializeField]
        [Tooltip("에디터 테스트 중 Host 없이 주방 레벨 변경을 허용할지 여부입니다.")]
        ...
        public bool TryUpgradeWithInventory(IInventory inventory)
        {
            ...
            if (!HasAllMaterials(inventory, nextLevelData))
            {
                LogWarning($"주방 Lv.{nextLevelData.level} 업그레이드 재료가 부족합니다.");
                return false;
            }

            if (!CanApplyUpgradeLevel())
            {
                LogWarning("현재 실행 상태에서는 주방 레벨을 변경할 수 없습니다. Host 실행 또는 Offline Test 허용 여부를 확인하세요.");
                return false;
            }

            if (!ConsumeAllMaterials(inventory, nextLevelData))
            {
                LogWarning($"주방 Lv.{nextLevelData.level} 업그레이드 재료 소모에 실패했습니다.");
                return false;
            }
            ...
            if (!ApplyUpgradeLevel(nextLevelData.level))
            {
                ...
                LogWarning("주방 레벨 적용에 실패했습니다. 소모한 재료를 되돌렸습니다.");
                return false;
            }
            ...
            if (logUpgradeResult)
                Debug.Log($"[KitchenUpgradeController] 주방 업그레이드 성공: Lv.{nextLevelData.level}", this);
        ...
        private bool IsValidKitchenFacility()
        {
            ...
            {
                LogWarning($"연결된 시설 타입이 Kitchen이 아닙니다. 현재 타입: {kitchenFacility.Type}");
                return false;
            }

            if (kitchenFacilityData == null)
            {
                LogWarning("Kitchen_Facility SO가 연결되어 있지 않습니다.");
                return false;
            }

            if (kitchenFacilityData.type != FacilityType.Kitchen)
            {
                LogWarning($"Kitchen_Facility SO의 타입이 Kitchen이 아닙니다. 현재 타입: {kitchenFacilityData.type}");
                return false;
            }
        ...
        private bool TryGetNextLevelData(out FacilityLevel nextLevelData)
        {
            ...
            if (kitchenFacilityData.levels == null || kitchenFacilityData.levels.Length == 0)
            {
                LogWarning("Kitchen_Facility SO에 레벨 데이터가 없습니다.");
                return false;
            }

            int currentLevel = GetCurrentKitchenLevel();
            int nextLevel = currentLevel + 1;

            if (nextLevel > kitchenFacilityData.levels.Length)
            {
                LogWarning("주방이 이미 최대 레벨입니다.");
                return false;
            }

            nextLevelData = kitchenFacilityData.GetLevel(nextLevel);

            if (nextLevelData == null)
            {
                LogWarning($"주방 Lv.{nextLevel} 데이터가 없습니다.");
                return false;
            }
        ...
        private bool ApplyUpgradeLevel(int nextLevel)
        {
            ...
            {
                kitchenFacility.CurrentLevel.Value = nextLevel;
                return true;
            }
            ...
            if (ShouldUseOfflineTestLevel())
            {
                ...
                {
                    Debug.Log(
                        $"[KitchenUpgradeController] 오프라인 테스트 모드로 주방 레벨을 변경했습니다. Lv.{offlineCurrentLevel}",
                        this
                    );
                }
            ...
            LogWarning("주방 레벨 변경은 서버에서만 가능합니다. Host 모드로 실행했는지 확인하세요.");
            return false;
        }
        ...
        private bool CanWriteNetworkLevel()
        {
            ...
            if (kitchenFacility.NetworkObject == null)
                return false;

            return kitchenFacility.NetworkObject.IsSpawned;
        }
        ...
        private void LogWarning(string message)
        {
            ...
            Debug.LogWarning($"[KitchenUpgradeController] {message}", this);
        }
        ...
        private void DebugCanUpgrade()
        {
            ...
            Debug.Log($"[KitchenUpgradeController] 업그레이드 가능 여부: {canUpgrade}", this);
        }
