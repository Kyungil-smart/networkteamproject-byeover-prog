...
public class KHWLootContainer : NetworkBehaviour, IInteractable
{
    ...
    public void TransferSlotToPlayerInventoryServerRpc(int slotIndex, ServerRpcParams rpcParams = default)
    {
        ...
        {
            Debug.LogWarning("[KHWLootContainer] 유효하지 않은 슬롯 인덱스: " + slotIndex, this);
            return;
        }
        ...
        if (slot.IsEmpty)
        {
            Debug.LogWarning("[KHWLootContainer] 빈 슬롯입니다: " + slotIndex, this);
            return;
        }
        ...
        if (!NetworkManager.Singleton.ConnectedClients.TryGetValue(senderClientId, out client))
        {
            Debug.LogWarning("[KHWLootContainer] ClientId를 찾을 수 없습니다: " + senderClientId, this);
            return;
        }

        if (client.PlayerObject == null)
        {
            Debug.LogWarning("[KHWLootContainer] PlayerObject가 없습니다: " + senderClientId, this);
            return;
        }
        ...
        Debug.Log("[KHWLootContainer] 슬롯 이동 / 슬롯=" + slotIndex
            + " / itemID=" + item.itemID
            + " / 수량=" + slot.amount
            + " / 결과=" + added, this);
